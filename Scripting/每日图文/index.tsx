...
const VERSION = "1.1.0"
const VERSION_ACK_KEY = "daily_quote_ack_version"

/** 数据变更后立即让桌面组件重新渲染，避免等 iOS 的下一个刷新窗口 */
...
function MainPage() {
  ...
  useEffect(() => {
    ...
    loadData()
    // 版本更新说明：首次打开新版本弹出一次底部 Sheet
    if (Storage.get<string>(VERSION_ACK_KEY) !== VERSION) {
      setShowUpdate(true)
    }
  }, [])
  ...
  async function loadData() {
    ...
    try {
      ...
      if (fixedImgPath && await FileManager.exists(fixedImgPath)) {
        ...
        if (result.image) changed = true
      }

      // 加载一言
      const fixedHkRaw = Storage.get<string>(FIXED_HK_KEY)
      if (fixedHkRaw) {
        try { setHitokoto(JSON.parse(fixedHkRaw)) } catch { setHitokoto(await fetchHitokoto()); changed = true }
      } else if (await FileManager.exists(metaPath)) {
        try {
          const meta = JSON.parse(await FileManager.readAsString(metaPath))
          setHitokoto(meta.hitokoto)
        } catch { setHitokoto(await fetchHitokoto()); changed = true }
      } else {
  ...
  function handleUnpin() {
    ...
    const fixedPath = `${dir}/daily_quote_fixed.jpg`
    ;(async () => {
      try { if (await FileManager.exists(fixedPath)) await FileManager.remove(fixedPath) } catch {}
    })()
    notifyWidgets()
    alert({
      title: "已取消固定",
    ...
    <NavigationStack
      sheet={{
        ...
        onChanged: (value: boolean) => {
          setShowUpdate(value)
          // 用户关闭后才记录已读版本，避免下次打开重复弹
          if (!value) Storage.set(VERSION_ACK_KEY, VERSION)
        },
        content: <UpdateSheetContent onClose={() => setShowUpdate(false)} />,
      }}
    >
      <List
...
function UpdateSheetContent({ onClose }: { onClose: () => void }) {
  return (
    <VStack
      spacing={14}
      padding={20}
    ...
    >
      <Image systemName="sparkles" font="title2" foregroundStyle="systemPurple" />
      <Text font="headline">每日图文 1.1.0 更新</Text>
      <VStack spacing={8} padding={{ horizontal: 6 }}>
        <Text font="footnote" foregroundStyle="secondaryLabel" lineSpacing={2}>
          · 修复桌面小组件偶发不显示/空白：渲染逻辑对齐「One 每日一言」稳定实现，缓存优先、网络超时保护、任何情况都有内容兜底
        </Text>
        <Text font="footnote" foregroundStyle="secondaryLabel" lineSpacing={2}>
          · App 内刷新 / 固定 / 取消固定后，桌面组件立即同步更新，不用再等 4 小时
        </Text>
        <Text font="footnote" foregroundStyle="secondaryLabel" lineSpacing={2}>
          · 小组件现在真正支持「固定图文」
        </Text>
      </VStack>
      <Button title="知道了" action={onClose} />
    </VStack>
  )
}

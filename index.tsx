...
import { fallbackRepositories, loadRepositories } from './data'
...
function RepositoryDetail({ repository }: { repository: Repository }) {
  return (
    <List navigationTitle={repository.name} navigationBarTitleDisplayMode="inline">
      <Section header={<Text>{repository.full_name}</Text>}>
        <Text>{repository.description || '暂无仓库描述。'}</Text>
      </Section>
      ...
      <Section>
        <Button title="在 GitHub 中打开" systemImage="safari" action={() => Safari.present(repository.html_url)} />
      </Section>
    </List>
  )
}
    ...
    <NavigationLink destination={<RepositoryDetail repository={repository} />}>
      <VStack alignment="leading" spacing={5}>
        ...
        <HStack spacing={14} foregroundStyle="secondaryLabel">
          <Label title={repository.language || '未知'} systemImage="chevron.left.forwardslash.chevron.right" />
  ...
  return (
    <NavigationStack>
      <List
        ...
        listStyle="plain"
        searchable={{ value: searchTerm, onChanged: setSearchTerm, prompt: '搜索仓库、描述或语言' }}
        refreshable={refresh}
        toolbar={
          <Button
            title="切换排序"
      ...
      >
        ...
        >
          {visibleRepositories.map(repository => <RepositoryRow key={repository.id} repository={repository} />)}
          {!isLoading && visibleRepositories.length === 0 ? <Text>没有匹配的仓库</Text> : null}
        </Section>
      </List>
    </NavigationStack>
  )
...
async function run() {
  await Navigation.present({ element: <LibraryScreen />, modalPresentationStyle: 'fullScreen' })
  Script.exit()
}
